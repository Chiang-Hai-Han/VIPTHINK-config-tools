...
def main() -> int:
    parser = argparse.ArgumentParser(description="打开调试浏览器并监听小老师、知识点等相关接口。")
    ...
    output_path = OUTPUT_DIR / f"小老师接口监听_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    user_data_dir = Path(tempfile.mkdtemp(prefix="small_teacher_debug_chrome_"))
    chrome = find_chrome()
    ...
    print("已打开调试浏览器，请在里面登录并操作需要监听的功能。")
            ...
            if method == "Network.requestWillBeSent":
                ...
                while time.time() < end:
                    ...
                    if message and message.get("id") == cdp.next_id - 1:
                        ...
                        break
                if likely_target_request(url, request.get("postData", "") + "\n" + body):
                    captures.append({
                        "url": url,
                        "status": response.get("status"),
                        "request": request,
                        "responsePreview": body[:12000],
                    })
