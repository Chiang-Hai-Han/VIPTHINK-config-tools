...
OUTPUT_DIR = ROOT / "简案接口监听"
DEFAULT_PORT = 9224
DEFAULT_URL = "https://jy.vipthink.cn/#/teacher_new/plan?course_category_id=3203%2C3278&game_url=&order=asc&page=1&page_count=10&page_num=1&sort=cn.sort&subject=-1"
...
def likely_target_request(url: str, text: str) -> bool:
    combined = (url + "\n" + text).lower()
    keywords = [
        "plan",
        "teacher_new/plan",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="监听简案下载与上传相关接口。")
    ...
    parser.add_argument("--url", default=DEFAULT_URL, help="浏览器打开地址")
    args = parser.parse_args()

    OUTPUT_DIR.mkdir(exist_ok=True)
    output_path = OUTPUT_DIR / f"简案接口监听_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    user_data_dir = Path(tempfile.mkdtemp(prefix="jianan_debug_chrome_"))
    chrome = find_chrome()
    subprocess.Popen([
        str(chrome),
    ...
    print("已打开调试浏览器，请在里面登录并完成一次简案下载与上传。")
    print(f"监听 {args.seconds} 秒，结果会保存到：{output_path}")
    ...
    try:
        ...
        while time.time() < deadline:
            ...
            if method == "Network.requestWillBeSent":
                ...
                if not likely_target_request(url, text):
                    continue
                command_id = cdp.send("Network.getResponseBody", {"requestId": request_id})
                body = ""
                end = time.time() + 3
                while time.time() < end:
                    message = cdp.recv()
                ...
                print(f"已捕获候选接口 {len(captures)}：{url}")
    finally:
        output_path.write_text(json.dumps(captures, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"监听结束，结果文件：{output_path}")
    return 0
