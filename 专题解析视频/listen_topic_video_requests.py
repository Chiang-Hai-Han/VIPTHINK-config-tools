...
def main() -> int:
    parser = argparse.ArgumentParser(description="打开调试浏览器并监听专题解析视频相关接口。")
    ...
    output_path = OUTPUT_DIR / f"专题解析视频接口监听_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    user_data_dir = Path(tempfile.mkdtemp(prefix="topic_video_debug_chrome_"))
    chrome = find_chrome()
    ...
    print("已打开调试浏览器，请登录后台并手动操作一次专题解析视频复制。")
        ...
        while time.time() < deadline:
            ...
            if method == "Network.requestWillBeSent":
                ...
                if request.get("method") != "POST":
                    continue
                response_body_message_id = cdp.send("Network.getResponseBody", {"requestId": request_id})
                body = ""
                end = time.time() + 3
                while time.time() < end:
                    message = cdp.recv()
                    if message and message.get("id") == response_body_message_id:
                        result = message.get("result", {})
                        body = result.get("body", "")
                        break
                combined_text = request.get("postData", "") + "\n" + body
                if likely_target_request(url, combined_text):
                    captures.append({
