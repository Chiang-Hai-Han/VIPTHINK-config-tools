...
def likely_target_request(url: str, method: str, text: str) -> bool:
    """判断是否是与讲次配置相关的接口"""
    combined = (url + "\n" + text).lower()
    keywords = [
        # 讲次编辑相关
...
def main() -> int:
    parser = argparse.ArgumentParser(description="打开调试浏览器并监听讲次配置相关接口。")
    parser.add_argument("--port", type=int, default=DEFAULT_PORT, help="调试端口")
    parser.add_argument("--seconds", type=int, default=240, help="监听秒数")
    parser.add_argument("--url", default="https://jy.vipthink.cn/#/teacher_new/chapter_number", help="浏览器打开地址")
    args = parser.parse_args()

    OUTPUT_DIR.mkdir(exist_ok=True)
    output_path = OUTPUT_DIR / f"讲次接口监听_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    user_data_dir = Path(tempfile.mkdtemp(prefix="chapter_number_debug_chrome_"))
    chrome = find_chrome()
    ...
    print("已打开调试浏览器，请在里面登录并操作讲次配置功能。")
            ...
            if method == "Network.requestWillBeSent":
                ...
                while time.time() < end:
                    ...
                    if message and message.get("id") == cdp.next_id - 1:
                        ...
                        break
                combined_text = request.get("postData", "") + "\n" + body
                if likely_target_request(url, request.get("method", ""), combined_text):
                    captures.append({
                        "url": url,
